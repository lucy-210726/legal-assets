...
<div class="stat-card s-orange"><span class="stat-card-label">계약서 검토 · 대기</span><span class="stat-card-right"><span class="stat-card-num" id="sc-rev-pending">—</span><span class="stat-card-unit">건</span></span></div>
<div class="stat-card s-purple"><span class="stat-card-label">계약서 검토 · 진행중</span><span class="stat-card-right"><span class="stat-card-num" id="sc-rev-progress">—</span><span class="stat-card-unit">건</span></span></div>
<div class="stat-card s-green"><span class="stat-card-label">계약서 검토 · 합의완료</span><span class="stat-card-right"><span class="stat-card-num" id="sc-rev-agreed">—</span><span class="stat-card-unit">건</span></span></div>
</div>
...
</div><div class="contract-type-grid" id="contract-grid"></div></div>
...
</div><div class="field-section-title">문의 내용</div><div class="form-grid cols-1" style="margin-bottom:20px;"><div class="form-group"><label>제목 <span class="req">*</span></label><input type="text" id="inq-title" oninput="checkInquiryReady()"></div><div class="form-group"><label>상세 내용 <span class="req">*</span></label><textarea id="inq-content" rows="6" placeholder="문의 내용을 상세히 작성해 주세요." oninput="checkInquiryReady()"></textarea></div></div><div class="field-section-title">파일 첨부 <span style="font-weight:400;color:var(--text-muted);font-size:0.78rem;letter-spacing:0;">(선택)</span></div><div style="padding:0 0 24px;">
<div class="attach-zone" onclick="document.getElementById('inq-form-attach-input').click()" style="min-height:auto;padding:16px;">
<input type="file" id="inq-form-attach-input" multiple onchange="handleAttachSelect('inq-form')"><div class="attach-zone-icon" style="font-size:1.3rem;margin-bottom:4px;">📎</div><div class="attach-zone-text" style="font-family:var(--font);"><strong style="font-size:0.85rem;">클릭하거나 드래그하여 첨부</strong><br><span style="font-size:0.75rem;color:var(--gold);">모든 형식 · 파일당 최대 20MB · 링크로 전송 · 3영업일 후 자동 삭제</span>
</div></div><div class="attach-file-list" id="inq-form-attach-list"></div></div></div><div class="form-footer"><div class="form-footer-note"><strong>*</strong> 필수 항목</div><div class="btn-row"><button class="btn btn-ghost" onclick="showPage('home')">취소</button><button class="btn btn-gold" id="inquiry-btn" onclick="submitInquiry()" disabled>문의 전송</button></div></div></div>
</div></div>
`;
...
'<colgroup><col style="width:28px"><col style="width:180px"><col><col class="hide-mobile" style="width:190px"><col style="width:90px"></colgroup>' +
'<thead><tr>' +
...
<div class="inq-reply-section" id="inq-reply-section"><label>✏️ 답변 작성</label><textarea id="inq-reply-textarea" placeholder="답변 내용을 입력하세요..."></textarea><div class="attach-zone" onclick="document.getElementById('inq-attach-input').click()"><input type="file" id="inq-attach-input" multiple onchange="handleAttachSelect('inq')"><div class="attach-zone-icon">📎</div><div class="attach-zone-text"><strong>클릭하거나 드래그하여 첨부</strong><br><span style="font-size:0.75rem;">모든 형식 · 파일당 최대 20MB · 링크로 전송 · 3영업일 후 자동 삭제</span>
</div></div><div class="attach-file-list" id="inq-attach-list"></div><div class="inq-reply-footer"><p>답변 전송 시 Slack 및 이메일로 자동 발송됩니다.</p><div class="btn-row"><button class="btn btn-ghost" onclick="clearInqSel()">선택 해제</button><button class="btn btn-gold" id="inq-reply-btn" onclick="sendInqReply()">답변 전송 →</button></div></div></div>
<div class="inq-reply-section" id="inq-progress-section" style="display:none;"><div id="inq-progress-info" style="font-family:var(--font);font-size:0.82rem;color:#1a5fa8;background:#e0f0ff;border:1px solid #a8cff0;border-radius:8px;padding:10px 14px;margin-bottom:14px;"></div><div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding:12px 14px;background:var(--surface);border:1px solid var(--border);border-radius:10px;"><span style="font-family:var(--font);font-size:0.82rem;font-weight:600;color:var(--ink-3);white-space:nowrap;">👤 진행자 변경</span><select id="inq-assignee-select" style="flex:1;font-family:var(--font);font-size:0.85rem;padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;background:var(--white);color:var(--text);"><option value="">진행자 선택...</option></select><button onclick="doChangeAssignee()" style="font-family:var(--font);font-size:0.8rem;font-weight:600;padding:8px 16px;border-radius:8px;border:1.5px solid var(--gold);background:transparent;color:var(--gold);cursor:pointer;white-space:nowrap;transition:all 0.15s;">변경</button></div><label>✏️ 답변 작성</label><textarea id="inq-progress-textarea" placeholder="답변 내용을 입력하세요..."></textarea>
<div class="attach-zone" onclick="document.getElementById('inq-prog-attach-input').click()" style="min-height:auto;padding:16px;"><input type="file" id="inq-prog-attach-input" multiple onchange="handleAttachSelect('inq-prog')"><div class="attach-zone-icon" style="font-size:1.3rem;margin-bottom:4px;">📎</div><div class="attach-zone-text" style="font-family:var(--font);"><strong style="font-size:0.85rem;">클릭하거나 드래그하여 첨부</strong><br><span style="font-size:0.75rem;color:var(--gold);">모든 형식 · 파일당 최대 20MB · 링크로 전송 · 3영업일 후 자동 삭제</span>
</div></div><div class="attach-file-list" id="inq-prog-attach-list"></div><div class="inq-reply-footer"><p>답변 전송 시 Slack 및 이메일로 자동 발송됩니다.</p><div class="btn-row"><button class="btn btn-ghost" onclick="clearInqSel()">선택 해제</button><button class="btn btn-gold" id="inq-progress-reply-btn" onclick="sendInqReply('progress')">답변 전송 →</button></div></div></div>
...
<div id="rev-reply-section" style="padding:0 28px 20px;">
...
<button class="rev-quick-label" onclick="insertQuickLabel('일부 수정 사항 있으므로 상세 수정 내용은 수정본을 통해 확인 부탁드립니다.')">🔄 수정 사항</button>
</div>
</div>
...
<span style="font-family:var(--font);font-size:0.82rem;color:var(--text-muted);"><strong>클릭하거나 드래그하여 첨부</strong><br><span style="font-size:0.72rem;">PDF, Word, HWP 등 · 파일당 최대 20MB</span>
</span>
</div>
</div>
...
<th class="col-rev-confirmed hide-mobile">진행자</th>
</tr>
</thead>
...
<!-- 검토 의견 회신 이력 -->
<div id="myrev-reply-history-wrap" style="display:none;margin-top:16px;padding:0;">
<div style="border:1px solid var(--border);border-radius:12px;background:var(--surface);padding:16px 18px;">
<div style="font-family:var(--font);font-size:0.82rem;font-weight:700;color:var(--ink-3);margin-bottom:10px;">📨 검토 의견 회신 이력</div>
<div id="myrev-reply-history-content" style="font-family:var(--font);font-size:0.84rem;color:var(--text);line-height:1.7;white-space:pre-wrap;"></div>
</div>
</div>
</div>
</div>
...
<div style="display:flex;align-items:center;gap:10px;justify-content:center;">
<span style="font-size:1.1rem;">📎</span>
<span style="font-family:var(--font);font-size:0.82rem;color:var(--text-muted);"><strong>클릭하거나 드래그하여 첨부</strong><br><span style="font-size:0.72rem;">선택 · 파일당 최대 20MB</span>
</span>
</div>
</div>

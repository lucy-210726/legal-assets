...
var CONTRACTS_DATA = [
  ...
  // ══════════════════════════════════════════════════════════
  {
    ...
    { section: '변경사항' },
    { name: 'change_category', label: '변경 구분',  type: 'text',     required: true, hint: '※ 변경 사항이 1개일 때만 자동작성 가능합니다.<br>항목이 여러 개인 경우 양식 다운로드 후 직접 작성해주세요.' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'total_fee',         label: '계약금액 (부가세 별도)', type: 'text',  required: true, format: 'currency', hint: '예: 10,000,000' },
    
      { section: '대금 지급' },
      { name: 'invoice_date',      label: '세금계산서 발행일',   type: 'text',     required: true,  hint: '예: 매월 말일, 데이터 공급월 말일 등' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'total_fee',      label: '총 이용료 (원)',    type: 'number', required: true, format: 'currency' },
      { name: 'total_month_fee', label: '월 구독료 (원)',   type: 'number', required: true, format: 'currency' }, 
      { section: '지급 조건' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'tvindex_fee',     label: 'TV INDEX LITE 금액',  type: 'text', required: true, format: 'currency' },
      { name: 'etc_1',           label: 'TV INDEX 비고',       type: 'text', required: false },
      { name: 'account_fee',    label: '계정 추가 금액',       type: 'text', required: false, format: 'currency' },
      { name: 'etc_2',           label: '계정 추가 비고',      type: 'text', required: false },
      { name: 'discount_fee',   label: '할인적용 금액',        type: 'text', required: false, format: 'currency' },
      { name: 'etc_3',           label: '할인 비고',           type: 'text', required: false },
      { name: 'total_fee',      label: '총 이용료 (원)',       type: 'text', required: true, format: 'currency' },
      { name: 'total_month_fee', label: '월 이용료 (원)',      type: 'text', required: true, format: 'currency' },
      { section: '이용권한' },
  ...
  },
 {
  ...
  fields: [
    ...
    { name: 'payment_date_terms', label: '입금일',            type: 'text',     required: false, defaultValue: '세금계산서 발행일 기준 익월(M+1) 말일 (주말, 공휴일인 경우 익영업일 이내)' },

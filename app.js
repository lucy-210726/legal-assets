...
google.script.run.withSuccessHandler(function(result){ if (result && result.ok) resolve(result); else reject(new Error((result && result.error)||'시트 업데이트 실패')); }).withFailureHandler(function(err){ reject(new Error(err.message||'시트 업데이트 실패')); }).finalizeUpload({fileId:fileId,rowNum:r.rowNum,contractParty:r.contractParty,counterParty:r.counterParty,managementNo:r.managementNo,contractName:r.contractName,startDate:r.startDate||'',sessionToken:window.SESSION_TOKEN||''});
});
setProgress(100, '완료!');
  ...
  try {
    ...
    await new Promise(function(resolve, reject) {
      google.script.run
        ...
        .handleSubmitInquiry(name, dept, category, title, finalContent, userEmail, userSlackId, toList, ccList, window.SESSION_TOKEN || '');
    });
...
// ────────────────────────────────────────────────────────────
var LOGIN_POPUP_BASE = 'https://lucy-210726.github.io/legal-assets/login.html';

function openLoginPopup() {
  var w = 480, h = 600;
  var left = (window.screen.width  - w) / 2;
  var top  = (window.screen.height - h) / 2;
  // GAS 웹앱은 매번 바뀌는 googleusercontent.com iframe 에서 실행되므로,
  // 팝업이 토큰을 정확한 대상 origin 으로 되돌려주도록 현재 창의 origin 을 전달한다.
  var popupUrl = LOGIN_POPUP_BASE + '?parentOrigin=' + encodeURIComponent(window.location.origin);
...
payload.sessionToken = window.SESSION_TOKEN || '';
try {
var result = await new Promise(function(resolve, reject) {
